...
from model.model_for_hyperpramets_setting import GCN_GAT
...
def readAndCreateData(df):   
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        ...
        print("pt files have been found")
...
def trainfolds():
    ...
    for fold in tqdm(range(NO_OF_FOLDS)):
        ...
        trigger_times = 0
        the_last_loss = 100
        if not os.path.exists('saved_models_dataset_1_hyperparameters/'+savepath):
            os.makedirs('saved_models_dataset_1_hyperparameters/'+savepath)
        model_file_name = 'saved_models_dataset_1_hyperparameters/' + savepath +'model_' +  str(fold) +  '.model'
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            print(f'test accuracy: {accuracy_test:.4f}, test loss: {avg_loss_test:.4f}, '
              ...
              f'test precision:{precision_test:.4f}, test f1_score:{f1_score_test:.4f}')
